...
//@Service
//@Scope("prototype")
public class QueryTradComm extends abstractCommnuication{

	private cardtrade  tradeinfo;
	//private int counter=0;
	
	public QueryTradComm(){
		command=ProtocolInfo.READTRADERECORD;	//命令		
		data=new byte[4];						//数据
		for(int i=0;i<4;i++)
			data[i]=0x30;
		respLength=79;							//响应长度
		this.sendbuff=new byte[12];			//发送缓冲区
	...
	public int analyRecvbuff() {
		if(recvbuff[0]==78){//判断返回数据长度
			if(recvbuff[78]==ProtocolInfo.fnChecksumCalc(recvbuff,1,78)){	//校验和				
				...
				try{
					tradeinfo.setCardMoney(Double.parseDouble(info.substring(68,76))/100);
				}catch(Exception e){
...
//				System.out.println("消费次数: "+tradeinfo.getMileage());
	...
	public void setCounter(int counter) {
		...
		byte[] ar =String.format("%04d",counter).getBytes();
		int len=(ar.length>3?4:ar.length);
